...
class Magento_Test_Db_Mysql extends Magento_Test_Db_DbAbstract
{
    ...
    public function cleanup()
    {
        ...
        $cmd = sprintf('/usr/local/mysql/bin/mysql --protocol=TCP --host=%s --user=%s --password=%s %s < %s',
    ...
    {
        $cmd = sprintf('/usr/local/mysql/bin/mysqldump --protocol=TCP --host=%s --user=%s --password=%s'
            // DDL
    ...
    {
        $cmd = sprintf(
            '/usr/local/mysql/bin/mysql --protocol=TCP --host=%s --user=%s --password=%s %s < %s',
                escapeshellarg($this->_host),
                escapeshellarg($this->_user),
                escapeshellarg($this->_password),
                escapeshellarg($this->_schema),
